<script>
  ...
  const logger = createLogger('DebugSuffix');
  
  // Props - use our isDebugMode store to determine if enabled
  ...
  function debugSuffix() {
    ...
    logger.info('Found app suffix elements:', suffixes.length);
    ...
    wrappers.forEach((wrapper, index) => {
      ...
      logger.debug(`Wrapper ${index} modified`);
    });
    
    // Check the app text elements
    const appTexts = document.querySelectorAll('.app-text');
    logger.info('Found app-text elements:', appTexts.length);
    ...
    appTexts.forEach((text, index) => {
      ...
      logger.debug(`Text ${index} content:`, text.textContent);
    });
  }
  ...
  function resetStyles() {
    ...
    logger.info('Debug styles removed');
  }
  
  // Lifecycle management
  onMount(() => {
    if (!enabled) {
      logger.debug('Debug tools disabled in production mode');
    }
  });
