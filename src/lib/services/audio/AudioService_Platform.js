...
async function requestIOSPermissions({ audioContext }) {
  ...
  if (audioContext?.state !== "running") {
    try {
      ...
      logger.warn("Failed to resume audio context for iOS", {
        error: err.message,
      });
    }
  }
  ...
  if (audioContext?.state !== "running") {
    logger.warn(
      "Audio context not running, but attempting to get permissions anyway",
    );
  }
  ...
  try {
    ...
    try {
      ...
      logger.error("iOS microphone permission denied", {
        errorMessage: iosFallbackError.message,
      });
      return { granted: false, error };
    }
  }
