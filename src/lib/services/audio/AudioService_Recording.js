...
export async function startRecording(context) {
  ...
  try {
    ...
    if (!granted) {
      ...
      throw (
        error ||
        new UIError("Microphone permission not granted", {
    ...
    try {
      ...
      await initializeVisualization(stream, audioContext);
...
export async function stopRecording(context) {
  const { stateManager, mediaRecorderRef, streamRef, audioChunksRef } = context;

  return new Promise((resolve) => {
    // Update recording state to STOPPING
    stateManager.setState(AudioStates.STOPPING);
