...
export class TranscriptionService {
  ...
  async transcribeAudio(audioBlob) {
    ...
    const sequence = ++this.transcriptionSequence;
    ...
    try {
      ...
      this.startProgressAnimation(sequence);
      ...
      this.completeProgressAnimation(sequence);
      ...
      const wordsPerMinute =
        transcriptText.split(/\s+/).length / (processingTime / 60000);
  ...
  startProgressAnimation(sequence) {
    this.cancelProgressAnimation();
    let progress = 0;
    ...
    const animate = () => {
      if (
        sequence !== this.transcriptionSequence ||
        !get(transcriptionState).inProgress
      )
        return;
      ...
      if (progress < 95) {
        this.progressTimer = setTimeout(animate, ANIMATION_INTERVAL);
      }
    };
  ...
  completeProgressAnimation(sequence) {
    if (this.progressTimer) {
      clearTimeout(this.progressTimer);
      this.progressTimer = null;
    }
    ...
    const complete = () => {
      ...
      if (progress < 99.5) {
        this.progressFrame = requestAnimationFrame(complete);
      } else {
