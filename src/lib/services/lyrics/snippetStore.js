    ...
    removeSnippet: (id) => {
      update((snippets) => {
        ...
        if (browser) {
          localStorage.setItem(
            "lyrics-snippets",
            JSON.stringify(reorderedSnippets),
          );
        }
      ...
      update((snippets) => {
        ...
        const updatedSnippets = snippets.map((snippet) => {
          if (snippet.id === id) {
            ...
            (oldOrder < boundedNewOrder &&
              snippet.order > oldOrder &&
              snippet.order <= boundedNewOrder) ||
            (oldOrder > boundedNewOrder &&
              snippet.order < oldOrder &&
              snippet.order >= boundedNewOrder)
          ) {
        ...
        const sortedSnippets = [...updatedSnippets].sort(
          (a, b) => a.order - b.order,
        );
