...
function blobToGenerativePart(blob) {
  return new Promise((resolve, reject) => {
    if (!blob || !(blob instanceof Blob)) {
      reject(
        new ApiError("Invalid blob provided", {
    ...
    reader.onloadend = () => {
      try {
        ...
        reject(
          new ApiError("Error processing blob", {
    ...
    reader.onerror = () => {
      reject(
        new ApiError("Error reading file", {
...
const generateContent = errorHandler.wrapAsync(
  async (promptData) => {
    try {
      const [requestOptions, audioPart] = Array.isArray(promptData)
        ? promptData
        : [promptData, null];
      const operation = requestOptions?.operation || "transcribeAudio";
      const promptStyle = requestOptions?.promptStyle || "standard";
      ...
      try {
        ...
        if (!response.ok) {
          throw new ApiError(
            data.error || "Failed to generate content with Gemini",
            {
